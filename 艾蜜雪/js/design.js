...
for (var p = 0; p < divs.length; p++){
  var pic = document.getElementsByClassName("pic")[p];
  huad();
}
function huad() {
  var pic_fir = pic.querySelector(".pic-fir");
  var j = 0;
  var u_li = pic.querySelectorAll("li");
  pic.onclick = function (e) {
    e = e || window.event;
    var target = e.target || e.srcElement;
    if (target.nodeName == "BUTTON"&&target.innerHTML=="&gt;") {
      j++;
      if (j == 2) {
        j = 0;
      }
      pic_fir.style.marginLeft = -998*j + "px";
      for (var i = 0; i < u_li.length; i++){
        u_li[i].className = "";
      }
      u_li[j].className = "active";
    } else if(target.nodeName == "BUTTON"&&target.innerHTML=="&lt;") {
      j--;
      if (j == -1) {
        j = 1;
      }
